...
/**
 * Frequency-domain steganography that embeds a secret image into a cover image using all three channels.
 * 
 * Both images are assumed to have the same resolution initially.
 * The secret image is downsampled by a given scaling factor before its data is embedded.
 * For each color channel the secret image (downsampled) is converted into a byte array (row-major order)
 * and then each bit is embedded into the cover image’s corresponding channel in the frequency domain.
 * Each frequency coefficient’s real part is nudged by a constant DELTA based on the bit value.
 * 
 * During decode the secret image’s bits are extracted from each channel, reassembled into the downsampled secret image.
 */
public class FourierSteganographyColor {

    // The amount by which to modify each coefficient’s real part per embedded bit.
    private static final double DELTA = 32000;

    /**
     * Encodes the secret image into the cover image.
     * The secret image is downsampled by the specified scale and its three color channels are embedded into
     ...
     * @param scale The scaling factor for downsampling the secret image (e.g., 0.5 for half resolution).
    ...
    public static Image encode(Image cover, Image secret, double scale) {
        ...
        // Each cover channel (frequency domain) has capacity equal to coverWidth * coverHeight coefficients.
        ...
        if (secretRData.length * 8 > capacity || 
            ...
            throw new IllegalArgumentException("Secret image (after downsampling) is too large to embed in cover image.");
        }
    ...
    /**
     * Converts a 2D int channel (dimensions [width][height]) to a byte array in row-major order.
     *
     ...
     * The frequency domain is represented as a double[][] with interleaved real and imaginary parts.
     *
    ...
    private static byte[] extractDataFromFrequencyDomain(double[][] freqDomain, int dataLength) {
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                if (bitIndex >= dataLength * 8) break outer;
                int realIndex = 2 * x;
                double realPart = freqDomain[y][realIndex];
    ...
    /**
     * Converts an int[][] channel (dimensions [width][height]) to a double[][] in interleaved complex format.
     * The resulting array has dimensions [height][2*width] with real parts at even indices.
     */
    ...
    /**
     * Converts an interleaved double[][] (from JTransforms) back to an int[][] channel.
